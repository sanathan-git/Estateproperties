...
const HouseContextProvider = ({ children }) => {
  ...
  const handleClick = () => {
    ...
    // get final value of price and parse it to number

    const minPrice = parseInt(price.split(' ')[0]);
   //get the second value of price which is the maximum price $ parse it to number
    ...
    const newHouses = housesData.filter((house)=> {
      ...
      // if country & property are default
      if(!isDefault(country) && !isDefault(property) && isDefault(price)){
